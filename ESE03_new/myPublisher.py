...
class MyPublisher():
    ...
    def setup(self):
        self.settings = json.load(open("settings_1.json", "r"))
        self.topic = self.settings["baseTopic"]
        self.broker = self.settings["broker"]
        self.port = self.settings["port"]
    
        self._paho_mqtt = PahoMQTT.Client(self.clientID, False) #FAlse means the client is a  durable client
    ...
    def publish(self, topic, msg, QoS):
        ...
        print ("publishing '%s' with topic '%s'" % (msg["status"], topic))
