...
class MyMQTT:
	def __init__(self, clientID, broker, port, notifier):
		...
		self._paho_mqtt = PahoMQTT.Client(clientID, False) #FAlse means the client is a  durable client
	...
	def myPublish (self, topic, msg):
		...
		self._paho_mqtt.publish(topic, msg, 2) #topic, msg = The payload to send, 2 = QoS level to use 

	def mySubscribe (self, topic):
		# if needed, you can do some computation or error-check before subscribing
		print ("subscribing to %s" % (topic))
		# subscribe for a topic
		self._paho_mqtt.subscribe(topic, 2) #arguments are topic and QoS
